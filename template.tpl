...
- name: Tag fires with valid config and click cookie
  code: |
    ...
    mock('sendHttpRequest', function(url, callback, options, body) {
      assertThat(url).contains('/v1/events/conversion');
      assertThat(options.headers['Authorization']).isEqualTo('Bearer sf_pub_test');
      var parsed = JSON.parse(body);
      assertThat(parsed.inf_click_id).isEqualTo('test_click_123');
      assertThat(parsed.organization_id).isEqualTo('test-org-id');
      assertThat(parsed.order_id).isEqualTo('ORDER-456');
      assertThat(parsed.revenue_cents).isEqualTo(4999);
      callback(200);
    });
    runCode(mockData);
...
- name: Tag skips when no click ID and auto mode
  code: |
    ...
    mock('getQueryParameters', function(param) { return null; });
    mock('copyFromDataLayer', function(key) {
      if (key === 'event') return 'page_view';
      return undefined;
    });
    mock('sendHttpRequest', function() {
      fail('sendHttpRequest should not be called without attribution');
    });
    runCode(mockData);
    assertApi('gtmOnSuccess').wasCalled();

- name: Tag sends custom event with explicit config
  code: |
    mock('getCookieValues', function(name) { return []; });
    mock('copyFromDataLayer', function(key) { return undefined; });
    mock('sendHttpRequest', function(url, callback, options, body) {
      assertThat(url).contains('/v1/events/custom');
      var parsed = JSON.parse(body);
      assertThat(parsed.event_name).isEqualTo('lead_form_submit');
      callback(200);
    });
    var customData = {
      ...
      orgId: 'test-org-id',
      eventType: 'custom',
      customEventName: 'lead_form_submit',
